...
\begin{itemize}
    \item we develop the method of controllable generation for several simultaneous attributes such as dialog acts and sentiment;
...
There are many different approaches to control generation process, one of them was proposed by Adapter bot \cite{madotto2020adapterbot} model which has an option of switch between different attributes without changes in initial model by adding adapter layers. Hyperformer \cite{mahabadi2021parameter} utilizes a shared PAL parameters for all tasks and Transformer layers, these parameters are generated by a hypernetwork. The model \cite{xie2021empathetic} is an encoder-decoder Transformer, where emotions in response are controlled with emotion embeddings, fed into the model. The limitation of hyperformer, adapter bot and \cite{xie2021empathetic} is inability to mix different attributes in one response (e.g., topic and emotion). In XiaoIce dialogue assistant \cite{zhou2019design} GRU is used to generate responses, conditioned on query and response empathy vectors. CRAYON \cite{hu2021controllable} is the model for multi-attribute response generation (response length, question/statement, sentiment, response relatedness). Our models generates responses for more dialog acts (not only question/statement) and does not require training the base model. \par
